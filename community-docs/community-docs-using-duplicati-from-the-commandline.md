# Community docs: using duplicati from the commandline

This page is copied from the community documentation: [See the original version of this page](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/)

### Introduction to the Duplicati Command Line tool[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#introduction-to-the-duplicati-command-line-tool) <a href="#introduction-to-the-duplicati-command-line-tool" id="introduction-to-the-duplicati-command-line-tool"></a>
...
### How to use the Duplicati Command Line tool[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#how-to-use-the-duplicati-command-line-tool) <a href="#how-to-use-the-duplicati-command-line-tool" id="how-to-use-the-duplicati-command-line-tool"></a>
...
* **Target URL**\
  ...
  Each storage provider has its own set of required and optional parameters. See [Storage Providers](https://prev-docs.duplicati.com/en/latest/05-storage-providers) for more info about specific backends.
* **Command arguments**\
...
> ![](https://prev-docs.duplicati.com/en/latest/icon_info.png) Advanced options with a value are preceeded with 2 dashes (`--`), followed by an equal=sign (`=`) and the desired value:\
...
> ![](https://prev-docs.duplicati.com/en/latest/icon_important.png) When using the command line tools, if any value (like a password) contains one or more special characters, enclose that value in double quotes or URL encode the special characters in that value. Keep in mind that this is not necessary when using the Web UI, as the Web UI will automatically convert special characters in the background. In that case, quotes will be considered as a literal part of the value.

***

***

> ![](https://prev-docs.duplicati.com/en/latest/icon_info.png) From this point, only Duplicati.CommandLine.exe will be used as reference to the Commandline tool. Linux and Mac OS X users should replace this with duplicati-cli or mono Duplicati.CommandLine.exe.

***

### Getting help from the Command Line Tools[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#getting-help-from-the-command-line-tools) <a href="#getting-help-from-the-command-line-tools" id="getting-help-from-the-command-line-tools"></a>
...
![](https://prev-docs.duplicati.com/en/latest/ss_gethelp_01.png)
...
![](https://prev-docs.duplicati.com/en/latest/ss_gethelp_02.png)

***

> ![](https://prev-docs.duplicati.com/en/latest/icon_info.png) The special help topic `example` will show a few examples of how the Commandline tool should be used to perform some simple operations.\
> To show the examples, type `Duplicati.CommandLine.exe help example`.

***

### The BACKUP command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-backup-command) <a href="#the-backup-command" id="the-backup-command"></a>
...
> ![](https://prev-docs.duplicati.com/en/latest/icon_info.png) Instead of composing the complete backup command yourself, including all advanced options, you can create a backup job in the Graphical User Interface, without scheduling it. Once completed, you can export the backup job to the command line, resulting in a Duplicati.CommandLine.exe backup command with all settings that you specified in the wizard. You can paste this generated command in your favorite task scheduler. For more information about creating a backup job in the Graphical User Interface, see [Creating a new backup job](https://prev-docs.duplicati.com/en/latest/03-using-the-graphical-user-interface/#creating-a-new-backup-job). For more information about exporting the backup job to the command line, see [Exporting a backup job configuration](https://prev-docs.duplicati.com/en/latest/03-using-the-graphical-user-interface/#exporting-a-backup-job-configuration).

***

The Commandline equivalent of the backup job described in [Creating a new backup job](https://prev-docs.duplicati.com/en/latest/03-using-the-graphical-user-interface/#creating-a-new-backup-job) is:
...
### The RESTORE command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-restore-command) <a href="#the-restore-command" id="the-restore-command"></a>
...
### The FIND command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-find-command) <a href="#the-find-command" id="the-find-command"></a>
...
### The COMPARE command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-compare-command) <a href="#the-compare-command" id="the-compare-command"></a>
...
### The TEST command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-test-command) <a href="#the-test-command" id="the-test-command"></a>
...
### The COMPACT command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-compact-command) <a href="#the-compact-command" id="the-compact-command"></a>
...
### The DELETE command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-delete-command) <a href="#the-delete-command" id="the-delete-command"></a>
...
### The PURGE command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-purge-command) <a href="#the-purge-command" id="the-purge-command"></a>
...
### The REPAIR command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-repair-command) <a href="#the-repair-command" id="the-repair-command"></a>
...
Example for B2 storage: `Duplicati.CommandLine.exe repair "b2://blacksmith/photos?auth-username=1234567890&auth-password=123ABc456dEfGH" --dbpath="C:\Users\User1\AppData\Local\Duplicati\SONSONSHE.sqlite" --passphrase="1234567890ABCdef"`

### The AFFECTED command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-affected-command) <a href="#the-affected-command" id="the-affected-command"></a>
...
### The LIST-BROKEN-FILES command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-list-broken-files-command) <a href="#the-list-broken-files-command" id="the-list-broken-files-command"></a>
...
### The PURGE-BROKEN-FILES command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-purge-broken-files-command) <a href="#the-purge-broken-files-command" id="the-purge-broken-files-command"></a>
...
### The CREATE-REPORT command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-create-report-command) <a href="#the-create-report-command" id="the-create-report-command"></a>
...
### The TEST-FILTERS command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-test-filters-command) <a href="#the-test-filters-command" id="the-test-filters-command"></a>
...
### The SYSTEM-INFO command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-system-info-command) <a href="#the-system-info-command" id="the-system-info-command"></a>
...
### The SEND-MAIL command[¶](https://prev-docs.duplicati.com/en/latest/04-using-duplicati-from-the-command-line/#the-send-mail-command) <a href="#the-send-mail-command" id="the-send-mail-command"></a>
...
Allowed placeholders are: \* `%PARSEDRESULT%`\
The parsed result op the operation: `Success`, `Warning`, `Error` \* `%RESULT%`\
When used in the body, this is the result/log of the backup,\
When used in the subject line, this is the same as `%PARSEDRESULT%` \* `%OPERATIONNAME%`\
The name of the operation, usually `backup`, but could also be `restore` etc. \* `%REMOTEURL%`\
The backend url. \* `%LOCALPATH%`\
